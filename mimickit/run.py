...
# ===============================================================
# 🧩 Variable & Argument Reference Table
# ===============================================================
#
# | Name               | Type / Example Value                  | Description |
# |--------------------|---------------------------------------|--------------|
# | argv               | list, sys.argv                        | Command-line arguments passed to the script. |
# | args               | ArgParser object                      | Parsed arguments loaded from command line or YAML file. |
# | arg_file           | str, "configs/args.yaml"              | Optional file containing argument values. |
# | env_config         | str, "configs/env.yaml"               | Path to environment configuration file. |
# | agent_config       | str, "configs/agent.yaml"             | Path to agent configuration file. |
# | mode               | str, "train" or "test"                | Operation mode (training or testing). |
# | num_envs           | int, 1                                | Number of environments per process. |
# | visualize          | bool, True                            | Whether to render environment during run. |
# | logger_type        | str, "tb" or "txt"                    | Type of logging backend (TensorBoard, text, etc.). |
# | log_file           | str, "output/log.txt"                 | Log output file path. |
# | out_model_file     | str, "output/model.pt"                | File path to save the trained model. |
# | int_output_dir     | str, "output/intermediate"            | Directory for intermediate or checkpoint files. |
# | model_file         | str, "output/model.pt"                | Pretrained model file to load. |
# | rand_seed          | int, 12345 or generated by time       | Random seed for reproducibility. |
# | master_port        | int, 6000–7000                        | Port used for inter-process communication. |
# | num_workers        | int, 1 or more                        | Number of worker processes. |
# | device             | str, "cuda:0" or "cpu"                | Device used for computation. |
# | rank               | int, 0, 1, 2, ...                     | Worker process rank ID. |
# | num_procs          | int, same as num_workers              | Total number of processes used. |
# | output_dir         | str, "output/"                        | Directory for final model output. |
# | out_model_dir      | str, extracted from out_model_file    | Directory containing saved model and configs. |
# | env_file           | str, "configs/env.yaml"               | Path to environment configuration. |
# | agent_file         | str, "configs/agent.yaml"             | Path to agent configuration. |
# | max_samples        | int, e.g., 1e6                        | Maximum number of samples for training. |
# | test_episodes      | int, e.g., 10                         | Number of test episodes. |
# | result             | dict, {"mean_return": ..., "mean_ep_len": ...} | Performance statistics from testing. |
# | processes          | list of Process                       | List of worker processes spawned. |
# | curr_device        | str, e.g., "cuda:1"                   | Device used by each worker process. |
# | root_rank          | int, 0                                | Rank of the main process. |
# | in_path            | str, e.g., "configs/env.yaml"         | Source file to be copied. |
# | out_filename       | str, e.g., "env_config.yaml"          | Destination filename after copying. |
# | output_dir (copy)  | str, e.g., "output/model_dir"         | Destination directory for copied files. |
# | out_file           | str, combined file path               | Full destination file path. |
#
# ===============================================================
...
def build_env(args, num_envs, device, visualize):
    env_file = args.parse_string("env_config") #parameter_wwj deepmimic_humanoid_env.yaml
...
def run(rank, num_procs, device, master_port, args):
    ...
    logger_type = args.parse_string("logger", "wandb")
